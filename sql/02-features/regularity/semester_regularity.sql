...
drop table if exists features.semester_regularity_dow;
create table if not exists  features.semester_regularity_dow as (
  with dow_raw as (
    select distinct
    student,
...
--hours per day
drop table if exists features.semester_regularity_hours_per_day;
create table if not exists  features.semester_regularity_hours_per_day as (
...
drop table if exists features.semester_regularity_frequency_hod;
create table if not exists  features.semester_regularity_frequency_hod as (
  with hod_raw as (
    select distinct
    student,
...
drop table if exists features.semester_regularity_hod;
create table if not exists  features.semester_regularity_hod as (
  with hod_raw as (
    select distinct
    student,
    extract(hour from event_date) as hod,
...
drop table if exists features.semester_regularity_frequency_days;
create table if not exists  features.semester_regularity_frequency_days as (
  with hod_raw as (
    select distinct
    student,
    case when extract(doy from event_date)::int >= 213 --August 1rst
